...
namespace SRP_DAL
{
    public class ActivityFeed
    {
        ...
        private const string query = "SELECT TOP 5" +
            ...
            " INNER JOIN [Patron] p ON pp.[PID] = p.[PID] AND p.[TenID] = @TenId AND " +
            "(p.[SuppressFromFeed] != 1 OR p.[SuppressFromFeed] IS NULL) " +
        ...
        public DataTable Latest(int after, int tenantId)
        { //, int tenId
