    ...
    public partial class Configure : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect("~/ControlRoom/", true);
            }
            catch (Exception ex)
            {
            ...
            // no database but 'configured' is set to 1 then redirect back out to front of site
            // this probably means the database is not accessible
            var configured = ConfigurationManager.AppSettings[AppSettingKeys.GraConfigured.ToString()];
            if (configured == "1")
            {
                this.Log().Error("Configure.aspx was loaded, could not connect to find programs, but the Web.config shows {0} is 1. Redirecting user to front of site - likely database is down",
                    AppSettingKeys.GraConfigured.ToString());
                Response.Redirect("~/TechnicalDifficulties.aspx", true);
            }
            ...
            else
            {
                ...
                // initial load, try reading and writing Web.config to ensure we can
                if (configured != "0")
                {
                    var issues = WriteWebConfigTest();
                    if (issues != null && issues.Count > 0)
                    {
                        ...
                        {
                            this.Log().Error("Initial configuration error: {0}", issue);
                            sb.AppendFormat("<li>{0}</li>", issue);
                        }
                        sb.Append("</ul>");
                        InitialStartMessage.Text = sb.ToString();
                        InitialStartPanel.Visible = true;
                    }
                    else
                    {
                        InitialStartPanel.Visible = false;
                    }
                }
            }
        }
        ...
        protected List<string> WriteWebConfigTest()
        {
            var issues = new List<string>();
            ...
            try
            {
                ...
                {
                    appSection.Settings.Add(AppSettingKeys.GraConfigured.ToString(), "0");
                }
                else
                {
                    appSection.Settings[AppSettingKeys.GraConfigured.ToString()].Value = "0";
                }
